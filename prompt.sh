...
printPrompt() {
  ...
  composeGitPart() {
    ...
    then
      ...
      BRANCH="${BRANCH:2}"
      echo -n "$SPACE${FG_BROWN}${OPEN_SQ_BRAQCKET}${FG_CYAN}$BRANCH"
      # staged files
     ...
     # unpushed commits
      ...
      if [[ $changeIndicator ]]
      then
        echo -n "${FG_RED}$SPACE$changeIndicator"
      fi
      if [[ "$stashStackDepth" != "0" ]]
      then
        echo -n "${FG_RED}$SPACE$stashStackDepth"
      fi
      echo -n "${FG_BROWN}${CLOSE_SQ_BRACKET}"
    else
      echo -n ""
    fi
  }
