...
printPrompt() {
  ...
  COMMAND_HOST='$(getValue HOST_ABBREVIATION ${HOST})'
  COLOR_HOST='$(getValue COLORS_HOST ${HOST})'
  ...
  composeBeginning() {
    ...
    # alma="$(getPart "${COLOR_HOST}${COLOR_PROMPT_BG}" "${COMMAND_HOST}")"
  ...
  composeEnd() {
    ...
    echo -n "${FG_GREEN}($(date -u -r ${EXEC_TIME} "+%M:%S")${NO_COLOR} "    
